...
export default function AboutPage() {
  return (
    <div className="content-layer flex h-full flex-col justify-between p-4 sm:p-8">
      ...
      <div className="mt-4 grid gap-3 sm:mt-7 sm:gap-5 md:grid-cols-[1.1fr_0.9fr]">
        ...
        <div className="rounded-3xl border border-white/[0.08] bg-white/[0.035] p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {stack.map((item) => (
              <span key={item} className="rounded-full border border-white/[0.08] bg-black/20 px-3 py-1.5 text-xs font-medium text-foreground/90">{item}</span>
            ))}
          </div>
